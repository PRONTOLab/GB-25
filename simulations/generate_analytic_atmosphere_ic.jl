#=
Generate a stand-in initial-condition JLD2 file for the 1° moist baroclinic
...
real spin-up checkpoint is on hand. Once the spin-up data is ready, drop the
...
# 1° atmosphere defaults — match the model's grid kwargs.
const Nλ = 360
const Nφ = 160
const Nz = 64
const H  = 30e3
const Δt = 60.0    # 60·(360/Nλ); the loader doesn't care, this is metadata only.

@info "Building 1° moist_baroclinic_wave_model on CPU (microphysics=nothing)..." now(UTC)
arch  = CPU()
...
mkpath(outdir)
outfile = joinpath(outdir, "atmosphere_no_microphysics_1deg_14day.jld2")
