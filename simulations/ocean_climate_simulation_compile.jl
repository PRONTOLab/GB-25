...
for debug in (true, false)
    open("before_raise_ocean_climate$(debug ? "_debug" : "").mlir", "w") do io
        show(IOContext(io, :debug => debug), unopt)
    end

    # Unoptimized HLO
    open("unopt_ocean_climate$(debug ? "_debug" : "").mlir", "w") do io
        show(IOContext(io, :debug => debug), unopt)
    end

    open("opt_ocean_climate$(debug ? "_debug" : "").mlir", "w") do io
