# Vanilla-Oceananigans 1/68° atmosphere run — 64 GPUs with NCCLDistributed.
# Initialized from 1/8° cascade checkpoint (interpolated ~8.5× in horizontal)
# with cloud fields for stable startup.
# Latitude range (-80, 80), Δt=0.2s, SST anomaly = +2 K.
#
# Grid: Nλ=24480, Nφ=10880, Nz=64 → ~17 billion cells
...
# RelaxToICForcing is inlined below (same as 1/24° script).
...
rank == 0 && @info "Starting NCCL-distributed 1/68° atmosphere simulation" nprocs now(UTC)
...
# 5-field variant: damps {ρu, ρv, ρw, ρθ, ρqᵛ}.
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# IC loader — each rank opens the full checkpoint, builds a GPU source
# grid at source resolution over (-80, 80), and interpolate!s into its
# local 1/68° portion.
# ═══════════════════════════════════════════════════════════════════════════
...
# Try the 1/8° checkpoint first, fall back to the coarsened file.
ic_dir = joinpath(@__DIR__, "..", "simulations", "initial_conditions")
ic_path = joinpath(ic_dir, "checkpoint_step_008193.jld2")
if !isfile(ic_path)
    ic_path = joinpath(ic_dir, "atmosphere_coarsened_1536x768x64.jld2")
end
isfile(ic_path) || error("IC file not found. Run: julia --project -O0 simulations/download_atmosphere_ic_artifact.jl")
rank == 0 && @info "IC file → 1/68°" ic_path

rank == 0 && @info "Building model (Nλ=$Nλ, Nφ=$Nφ, Nz=$Nz, lat=(-80,80), Δt=$(Δt)s, sst_anomaly=$(sst_anomaly)K)..." now(UTC)
...
rank == 0 && @info "Loading ICs (interpolating source → 1/68°)..." now(UTC)
...
output_dir = joinpath(@__DIR__, "..", "simulations", "output", "nccl_64gpu_68th_deg")
...
stop_iter = 54000  # 3h sim at Δt=0.2
...
simulation.output_writers[:fields] = JLD2Writer(model, output_fields;
    ...
    schedule = IterationInterval(100_000),  # effectively disabled
