#!/usr/bin/env julia
#
# Backward-only differentiable atmosphere — Reactant + Enzyme.
#
# Builds the model from IC, compiles and runs the Enzyme backward pass
# (no forward time-stepping — use forward_backward_sim.jl for that).
#
# Usage:
#   julia --project=.. backward.jl                        # 128×128×64 per GPU
#   julia --project=.. backward.jl --grid-x 64 --grid-y 64 --grid-z 32

using Dates
@info "Backward-only differentiable simulation starting" now(UTC)
...
out_dir = joinpath(@__DIR__, "output", "backward", ngpu_tag)
