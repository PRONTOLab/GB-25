...
xy_fields = [:ρ, :ρu, :ρv, :ρw, :ρθ, :ρqᵛ, :ρqᶜˡ, :ρqᶜⁱ]
all_z_levels = collect(1:Nz)
output_slices = [(f, :xy, all_z_levels) for f in xy_fields]
...
# ─── Phase 4: Production run at Δt=0.35 ──────────────────────────────
# No Δt ramp needed — already compiled at 0.35
@info "[$rank] Phase 4: production run at Δt=$Δt" now(UTC)

# 12 blocks × 5×1024 = 61440 steps × 0.35s = 21504s ≈ 5.97h sim time
# Save every ~30 min of sim time
const Nouter = 12
const Ncalls_per_block = 5   # 5 × 1024 = 5120 steps per block
steps_per_block = Ncalls_per_block * 64

@info "[$rank] Starting production: $Nouter blocks × $steps_per_block steps (Δt=$Δt)" now(UTC)
...
for k in 1:Nouter
    ...
    wall_block = (time_ns() - t0) / 1e9
    sim_time   = steps_per_block * k * Δt
    total_wall = (time_ns() - wall_start) / 1e9
    sypd       = (steps_per_block * Δt) / (365.25 * 86400 * wall_block) * 365.25

    @info @sprintf("[%d] block %d/%d  wall=%.1fs  sim=%.1fs  SYPD=%.5f  total_wall=%.0fs",
                    rank, k, Nouter, wall_block, sim_time, sypd, total_wall)

    # Save full 3D state every block (via z-level slices)
