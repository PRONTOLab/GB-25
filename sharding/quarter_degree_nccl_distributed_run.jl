# Vanilla-Oceananigans 1/4° atmosphere run — 8 GPUs with NCCLDistributed.
# Latitude range (-70, 70), Δt=1s.
#
# Launch: mpiexec -n 8 julia --project -O0 sharding/quarter_degree_nccl_distributed_run.jl
#
...
rank == 0 && @info "Starting NCCL-distributed quarter-degree atmosphere simulation" nprocs now(UTC)
...
# ═══════════════════════════════════════════════════════════════════════════
# IC loader — each rank loads the full file, sets its local portion
# ═══════════════════════════════════════════════════════════════════════════

function load_ic_distributed!(model, path::String; H = 30e3)
...
rank == 0 && @info "Loading ICs..." now(UTC)
@time "load ICs" load_ic_distributed!(model, ic_path; H=column_height)
...
output_dir = joinpath(@__DIR__, "..", "simulations", "output", "nccl_4gpu_3h_v2")
...
stop_iter = 11265  # ~3.1h with Δt=1s

simulation = Oceananigans.Simulation(model; Δt, stop_iteration=stop_iter)

simulation.output_writers[:fields] = Oceananigans.JLD2OutputWriter(model, output_fields;
    filename = output_prefix,
    schedule = Oceananigans.IterationInterval(1024),
...
function progress(sim)
    if rank == 0
        ...
        sdpd = (sim.model.clock.time / 86400) / (wall / 86400)
        @info @sprintf("iter %d, sim_time=%.0fs (day %.2f), SDPD=%.1f, wall=%.0fs",
                        sim.model.clock.iteration, sim.model.clock.time,
                        sim.model.clock.time / 86400, sdpd, wall)
    end
    flush(stderr); flush(stdout)
end

simulation.callbacks[:progress] = Oceananigans.Callback(progress, Oceananigans.IterationInterval(1024))

rank == 0 && @info "Starting simulation (stop_iter=$stop_iter, Δt=$Δt)" now(UTC)
wall_start[] = time_ns()
Oceananigans.run!(simulation)

rank == 0 && @info "Done!" now(UTC)
MPI.Finalize()
